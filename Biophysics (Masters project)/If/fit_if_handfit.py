...
voltage_range = np.arange(-120.0, -10.0, 10.0)

i_f_max = array([]) # Initialise the iv array
...
for volt in voltage_range:
        ...
        for time in arange(0, 6100+HT, HT):	
                ...
                a = 0.36*(svolt+148.8)/(exp(0.09*(svolt+148.8))-1.0)
                b  = 0.1*(svolt+87.3)/(1.0-exp(-0.05*(svolt+87.3)))
            ...
            #=======END Time Loop===============
                ...
                i=i+1
                if (i == 110000):
                        i_f_max=append(i_f_max, i_f)

        i=0

        
        abs_if=abs(i_f_max)
        max_i_f=abs_if[argmax(abs_if)]
        normal_i_f=i_f_max/max_i_f
...
#	print "p0", array(p0)
	...
	print "i_f_max", i_f_max
	print "normal_i_f", normal_i_f
	print "y_true", y_true

        plt.plot(x,normal_i_f,x,y_true, 'o')
	#Least Squares Fit
