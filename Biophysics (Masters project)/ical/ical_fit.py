...
def calculateical(p):
	...
	voltage_range = np.arange(-40.0, 70.0, 10.0)
	...
	for step in voltage_range:
		...
		for time in arange(0, 400+HT, HT):
				...
				svolt = -50.0
                        ...
                        am = 0.32*(svolt+47.13)/(1.0-exp(-0.1*(svolt+47.13)))
                        bm = 0.08*exp(-svolt/13)

                        ah = 0.0;
			bh = 1.0/(0.13*(1.0+exp((svolt+10.66)/-11.1)));
			aj = 0.0;
			bj = (0.3*exp(-0.0000002535*svolt))/(1.0+exp(-0.1*(svolt+32.0)))
		
                        h = ah/(ah+bh)-((ah/(ah+bh))-h)*exp(-HT/(1/(ah+bh)))
                        j = aj/(aj+bj)-((aj/(aj+bj))-j)*exp(-HT/(1/(aj+bj)))
                        m = am/(am+bm)-((am/(am+bm))-m)*exp(-HT/(1/(am+bm)))
        
                        ina = gna*m*m*m*h*j*(svolt-ena)
                        #======================================#

                        #=============compute ical=============#
                        dss = 1.0/(1.0+exp(-(svolt+p[0])/p[1]))
                        taud = (1.0-exp((svolt+p[0])/-6.24))/(0.035*(svolt+p[0])*(1.0+exp((svolt+p[0])/-6.24)))
		
                        fss = 1.0/(1.0+exp((svolt+p[2])/p[3]))
                        tauf = 9.0/(0.0197*exp(-pow((8.0*0.0337*(svolt+2.0)),2))+0.005)
		
                        fcass = 1.0/(1.0+cai/0.0035)
                        taufca = 2.0
                        ...
                        #=============comp jsr==============#
                        fn = vjsr*(1e-12)*ireljsrol-(1e-12)*caiont*acap/(2.0*frdy)
                        ...
                        wrelss = 1.0-1.0/(1.0+exp(-(svolt-40.0)/17))
                        ...
                        #=================================#
                        ical_array = append(ical_array, ilcatot);

			#=======END TIME LOOP=============#

                tail_ical = ical_array[31000:]
		max_tail_ical = tail_ical[argmax(tail_ical)]
		ical_iv = append(ical_iv, max_tail_ical)
...
def peval(parameters):
	y_approx = calculateical(parameters)
...
if __name__ == '__main__':
	# Read in data
	data = readfile('ical pacemaking PVpacemaking1.csv')
	voltages = data[0]
	wt = data[1]
...
##		
##			ikr = gkr*xr*r*(v-ekr)
##		    ikr_array = append(ikr_array, ikr );
##		    #=======END Time Loop===============
##			    
