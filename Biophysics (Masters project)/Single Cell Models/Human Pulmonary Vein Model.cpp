...
//        }
	...
	void comp_ina ()
	{
			...
			{ah = 0.0;
				bh = 1/(0.13*(1+exp((v+10.66)/-11.1)));//bh = 1/(0.13*(1+exp((v+10.66)/-11.1)));
				aj = 0.0;
	...
	{
		dss = 1/(1+exp(-(v-7)/6));
		taud = (1-exp((v-7)/-6.24))/(0.035*(v-7)*(1+exp((v-7)/-6.24)));
		
        fss = 1/(1+exp((v+28)/6.9));
        tauf = 9/(0.0197*exp(-pow((8*0.0337*(v+2)),2))+0.005);
		...
		ibarca = 0.710152141*gcalbar*(v-65);                
...
b  = 1068.0*exp(-(v+26.3)/30.0);
tau = 1.0/(a2+b);
inf = 1.0/(1.0+exp(-(v+37.0)/6.8));
dd = inf + (dd-inf)*exp(-dt/tau);

a2 = 15.3*exp(-(v+71.0)/83.3);
b  = 15.0*exp((v+71.0)/15.38);
tau = 1.0/(a2+b);
inf = 1.0/(1.0+exp((v+71.0)/9.0));
ff = inf + (ff-inf)*exp(-dt/tau);

icaT = GcaT*acap*gcaT*ff*dd*(v-EcaT);*/ // v = voltage or membrane potential
...
/*a2 = 0.36*(v+148.8)/(exp(0.066*(v+148.8))-1.0);
b  = 0.1*(v+87.3)/(1.0-exp(-0.21*(v+87.3)));
inf = a/(a+b);
tau = 1.0/(a+b);
aa = inf + (aa-inf)*exp(-dt/tau);

i_f = Gf*acap*gf*aa*(v-Ef);
	}*/

	
	friend std::ostream & operator<<(std::ostream& os, CRN& cell) {
		os	<< cell.v
			//<< std::setw(15) << cell.v << std::setw(15) << cell.ikr 
			//<< std::setw(15) << cell.iks << std::setw(15) << cell.iki 
			<< std::endl;
