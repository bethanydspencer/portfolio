...
max_inaca = array([])						# Initialise the iv array
i=0
#==================
# Run Voltage Clamp
#==================
for volt in voltage_range:
        ...
        for time in arange(0, 400+HT, HT):	
                ...
                #inaca = 4.90875669  *(exp(1.03210807*frdy*v/(R*temp))*nai*nai*nai*cao-exp((1.03210807-1)*frdy*v/(R*temp))*nao*nao*nao*cai)/((pow(-2.44215331,3)+pow(nao,3))*(-2.44215331+cao)*(1+(-1.12772186)*exp((1.03210807-1)*frdy*v/(R*temp))))
                inaca = 1750*(exp(gammas*frdy*v/(R*temp))*nai*nai*nai*cao-exp((gammas-1)*frdy*v/(R*temp))*nao*nao*nao*cai)/((pow(kmnancx,3)+pow(nao,3))*(kmcancx+cao)*(1+ksatncx*exp((gammas-1)*frdy*v/(R*temp))))#ikr_array = append(ikr_array, inaca )

                i=i+1

                if (i == 31000):
                        max_inaca=append(max_inaca, inaca)
...
##        tail_ikr = ikr_array[31000:]
        max_inaca_max = max_inaca[argmax(max_inaca)]
        normal_max_inaca=max_inaca/max_inaca_max
...
#	print "p0", array(p0)
	...
	plt.plot(x,max_inaca,x,y_true,'o')
